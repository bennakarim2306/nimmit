...
function Browse(props) {
    ...
    const [rangeTime, setRangeTime] = useState(15);
    ...
    const _handleSliderValueChanged = (value: number, slider: number) => {
        // console.log(`Browse -> _handleSliderValueChanged() -> ${value}`)
        const roundedValue = Math.round(value);
        if (slider === 1) {
            setRangeA(roundedValue);
        }
        if (slider === 2) {
            setRangeB(roundedValue);
        }
        if (slider === 3) {
            setRangeTime(roundedValue);
        }
    };
    ...
    const _dateTimePicker = (props) => {
        ...
        const styleInside =
            props.mode === 'date' ? {fontSize: 25} : {fontSize: 15};
        return (
                    ...
                    <Block style={{flex: 1, flexDirection: 'row'}}>
                        <Block style={styles.slider_container}>
                            <Slider
                                ...
                                value={rangeA}
                                onValueChange={(val) =>
                                    _handleSliderValueChanged(val, 1)
                                }
                    ...
                    <Block style={{flex: 1, flexDirection: 'row'}}>
                        <Block style={styles.slider_container}>
                            <Slider
                                ...
                                value={rangeB}
                                onValueChange={(val) =>
                                    _handleSliderValueChanged(val, 2)
                                }
                                onSlidingComplete={console.log('')}
                            />
                        </Block>
                ...
                <Block style={styles.routine_onetime_container}>
                    <Block style={{flexDirection: 'row', flex: 1}}>
                        ...
                        <Block style={styles.switch_container}>
                            ...
                            <Block flex={1}>
                                <Switch
                                    toggleFunction={_handleToggleSwitch}
                                    isEnabled={isRoutine}
                                />
                            </Block>
                            <Block flex={1}>
                ...
                <Block style={styles.date_time_container}>
                    ...
                    <Block style={styles.time_slider_container}>
                        <Slider
                            ...
                            value={rangeTime}
                            onValueChange={(val) =>
                                _handleSliderValueChanged(val, 3)
                            }
                            onSlidingComplete={console.log('')}
                        />
                    </Block>
                    <Block style={styles.range_container}>
                        <Text>{rangeTime} min</Text>
                    </Block>
                </Block>
                <Block style={styles.query_button_container}>
                    <Button gradient onPress={_handleQuery}>
                        {isSearching ? (
